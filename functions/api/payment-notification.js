export async function onRequestPost(context) {
  ...
  try {
    ...
    const WEBHOOK_ADMIN  = (context.env.DISCORD_WEBHOOK_ADMIN || '').trim();

    // Verifikasi signature dari Duitku
    // Format: MD5(merchantCode + amount + merchantOrderId + apiKey)
    const expectedSig = md5(merchantCode + amount + merchantOrderId + API_KEY);

    if (signature !== expectedSig) {
      console.error('Invalid signature from Duitku');
      console.error('Expected:', expectedSig);
      console.error('Got:', signature);
      return new Response('Invalid signature', { status: 403 });
    }

    console.log('Payment notification:', JSON.stringify(body));

    // Kirim notif ke Discord webhook admin
    if (WEBHOOK_ADMIN) {
      const isSuccess = resultCode === '00';

      const embed = {
        title: isSuccess ? '✅ Pembayaran Berhasil' : '❌ Pembayaran Gagal',
        color: isSuccess ? 0x4ade80 : 0xf87171,
        fields: [
          { name: '📦 Produk',      value: productDetail || '-',  inline: false },
          { name: '🆔 Order ID',    value: merchantOrderId || '-', inline: true  },
          { name: '💰 Jumlah',      value: 'Rp ' + Number(amount).toLocaleString('id-ID'), inline: true },
          { name: '🔖 Referensi',   value: reference || '-',       inline: true  },
          { name: '📊 Status',      value: isSuccess ? 'SUKSES' : 'GAGAL',        inline: true  },
        ],
        timestamp: new Date().toISOString(),
        footer: { text: 'OWO Cash Payment System' }
      };
